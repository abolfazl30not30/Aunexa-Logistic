...
function Group() {
    ...
    const handleChangeAllChecked = (event,item)=>{
        if(event.target.checked){
            const obj=item?.geoFenceIds
            let updateList=trackingMachineList?.filter((machine)=>  !item.geoFenceIds.find(vehicle => (vehicle.id === machine.id ) ))
                                                ...
                                                <summary className="flex items-center justify-between gap-2  font-medium marker:content-none hover:cursor-pointer px-2">
                                                    <div className="flex items-center py-1 gap-3">
                                                        <div className="text-gray70 whitespace-nowrap ">
                                                                <Checkbox
                                                                    id={item?.geoFenceIds[0]?.id}
                                                                    checked={item.geoFenceIds?.every((data) => trackingMachineList?.some((machine) => machine.id === data.id))}
                                                                    onClick={(e) => {handleChangeAllChecked(e, item);}}
                                                                    style={{
                                                                        width: 10,
                                                                        height: 10
                                                                    }}
                                                        ...
                                                        <div className="text-textGray  text-[0.9rem] text-sm">{item?.groupName}</div>
                                                    </div>
                                                        ...
                                                        (geo, index) => (
                                                            <li className="geofence-in-group flex justify-between  items-center px-2 py-2 border-b border-b-1 border-solid  border-borderGray">
                                                                <div className="gap-3 flex items-center">
                                                                    ...
                                                                    <span className="table-cell text-sm   text-gray70 whitespace-nowrap ">{geo?.name}</span>
                                                                </div>
                                                                <div>
